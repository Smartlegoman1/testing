...
module.exports = class extends BaseCommand {
    ...
    async run(client, message, args) {
        if (message.flags.includes('interactive')) {
            ...
            if (!channel.permissionsFor(message.guild.me).has(['SEND_MESSAGES'])) return message.channel.send('Oha doesn\'t have permissions to that channel, please give Oha access to that channel for this to work and try again...Exiting Setup');
        ...
        switch (key) {
            case 'channel': {
                ...
                if (!channel.permissionsFor(message.guild.me).has(['SEND_MESSAGES'])) return message.channel.send('Oha doesn\'t have permissions to that channel, please give Oha access to that channel for this to work and try again.');
                const model = message.guild.db.welcomer(true);
                model.channel = channel.id;
                await model.save();
